    ...
    in
        Options.apply summary
            ...
            [ styled button
                ...
                , cs "mdl-js-button"
                , cs "mdl-button--icon"
                , when (Internal.attribute (onKeyDown (Key itemSummaries))) <| isActive model
                , when (Internal.attribute (onClick Geometry.decode (Open))) <| model.animationState /= Opened
                , when (Internal.attribute (Html.Events.onClick Close)) <| isActive model
                ]
...
view1 lift config model offsetTop offsetHeight index summary item =
    ...
    in
        Options.apply summary
            ...
            , when (cs "mdl-menu__item--full-bleed-divider") summary.config.divider
            , when (css "background-color" "rgb(238,238,238)") <| model.index == Just index
...
pack : (Parts.Msg (Container b) m -> m) -> Parts.Index (List Int) -> Msg m -> m
pack =
