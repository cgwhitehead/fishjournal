...
headerView lift config model ( drawerButton, rows, tabs ) =
    ...
    in
        Options.styled Html.header
            ...
            , when (cs "is-animating") model.isAnimating
            , when (cs "is-compact") model.isCompact
            , mode
            , when (cs "mdl-layout__header--transparent") config.transparentHeader
            , Internal.attribute <|
                Events.onClick
        ...
        div
            ...
            [ filter (Keyed.node "div")
                ...
                , Options.styled main_
                    ...
                    , when (css "overflow-y" "visible") (config.mode == Scrolling && config.fixedHeader)
...
sub0 lift =
    Tuple.second init |> Cmd.map (pack lift)
